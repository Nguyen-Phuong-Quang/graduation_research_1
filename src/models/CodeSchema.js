    ...
    password: {
        ...
        validate: (value) => {
            if (!value.match(/^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])([a-zA-Z0-9]{8,})$/)) {
                throw new Error("Password must contain at least 8 characters, 1 lowercase letter, 1 uppercase letter and 1 number")
            }
        }
    },
    role: {
...
CodeSchema.pre('save', async function (next) {
    ...
    if (!this.password)
        await sendResetPassword(this.email, code);
    else
        await sendVerifyMail(this.email, code);
    this.code = code;
    next();
});
